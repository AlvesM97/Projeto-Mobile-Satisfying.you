...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, Dimensions } from 'react-native'; // Importando Dimensions
import Icon from 'react-native-vector-icons/MaterialIcons';
import Botao from '../components/Botao';

const RecuperarSenha = () => {
  const [email, setEmail] = useState("jurandir.pereira@hotmail.com");
  const [emailValid, setEmailValid] = useState(true); // Adicionando estado para emailValid

  const validarEmail = (text) => { //função validarEmail
    const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    setEmailValid(regex.test(text)); // Modificando para testar o texto passado como argumento
  };
    ...
    <View>
      ...
      <View style={styles.container}>
        ...
        <TextInput
          ...
          onChangeText={(text) => {
            setEmail(text);
            validarEmail(text); // Chama validarEmail enquanto o usuário digita
          }}
...
const styles = StyleSheet.create({ 
  ...
  buttonsContainer: {
    alignItems: 'center', // Centralize o botão
    marginTop: 20,
  },
});
